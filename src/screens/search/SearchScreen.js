  ...
  return (
    <View className="mt-2 mb-16">
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        {stores.map((item, i) => {
          return <ListCard navigation={navigation} key={i} details={item} />;
        })}
      </ScrollView>
    </View>
  );
...
const SearchScreen = ({ navigation }) => {
  ...
  const fetchSearchHistory = useCallback(async () => {
    ...
    try {
      ...
      const userRef = users.find((user) => user.id === uid);
      if (userRef) {
        const fetchedSearchhistory = userRef.search_history.reverse();
        setSearchHistory(fetchedSearchhistory);
      }
    }
  }, [auth.currentUser]);
