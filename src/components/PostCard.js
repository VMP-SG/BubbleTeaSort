...
const PostCard = ({ post, location, style, onPress }) => {
  ...
  useEffect(() => {
    const getStore = async () => {
      ...
      if (docSnap.exists()) {
        ...
        const distance = calcDistance(location.coords.latitude, location.coords.longitude, store.coordinates.latitude, store.coordinates.longitude);
  ...
  return (
    <TouchableOpacity className='bg-brown-500 rounded-xl' style={style} onPress={onPress}>
      <Image 
        source={imageUri === null ? require("../../assets/icons/Placeholder.png") : { uri: imageUri }}
        className='w-full h-60 rounded-xl'
      />
      <View className='p-3'>
        <Text className='font-secondary-bold' numberOfLines={1}>{post.title}</Text>
        <View className='flex-row justify-between mt-6 items-center'>
          ...
          <View className='flex-row items-center'>
            ...
            <Text className='font-secondary text-base ml-1'>{nFormatter(likeCount, 1)}</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>
