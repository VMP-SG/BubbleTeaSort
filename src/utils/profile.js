...
const getPastMonthPosts = (posts) => {
  ...
  const pastMonthPosts = posts.filter(post => post.timestamp.getTime() >= lastMonthDate.getTime());
...
export const getActivityPercentagesAndLabels = async (posts) => {
  ...
  posts.forEach((post) => {
    ...
    const index = storesPercentage.findIndex((storePercentage) => storePercentage.id === storeId);
  ...
  querySnapshot.forEach((doc) => {
    const data = doc.data();
    const id = doc.id;
    const index = storesPercentage.findIndex((storePercentage) => storePercentage.id === id);
    if (index !== -1) {
  ...
  storesPercentage.forEach((storePercentage) => {
    ...
    const index = brandsPercentage.findIndex((brandPercentage) => brandPercentage.brand === brand);
    if (index === -1) {
  ...
  if (fourth.length > 1) {
    const othersPercentage = fourth.reduce((sum, current) => sum + current.percentage,0)
    fourth = {"brand": "Others", percentage: othersPercentage};
  } else if (fourth.length === 0) {
    fourth = undefined;
    while (!fourth || fourth.brand === top.brand || fourth.brand === second.brand || fourth.brand === third.brand) {
      fourth = placeholders.shift();
    }
  }

  const labels = [top.brand, second.brand, fourth.brand, third.brand];
  const percentages = [top.percentage, second.percentage, fourth.percentage, third.percentage];
